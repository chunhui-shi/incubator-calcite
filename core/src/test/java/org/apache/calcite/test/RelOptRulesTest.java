...
public class RelOptRulesTest extends RelOptTestBase {
  ...
  final private PushProjector.ExprCondition skipItem = new PushProjector.ExprCondition() {
  ...
  @Test public void testPushProjectPastJoinWithExprCond() {
    PushProjector.ExprCondition skipItem = new PushProjector.ExprCondition() {
    ...
    ProjectJoinTransposeRule customPJTrans =
        new ProjectJoinTransposeRule(skipItem, RelFactories.LOGICAL_BUILDER);

/*    checkPlanningDynamic(customPJTrans,
        "select t1.c_nationkey, t2.fake_col['fake_col2'] "
        + "from SALES.CUSTOMER as t1, "
        + "unnest(t1.fake_col) as t2");
*/
    checkPlanningDynamic("select t1.c_nationkey, t2.fake_col2 "
            + "from SALES.CUSTOMER as t1, "
